...
function update_format(network)
    ...
    for i=1:length(network["bus"])
        bus_index=merge(bus_index, Dict(string(network["bus"][string(i)]["index"])=>i));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("gen"=>false));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("Pd"=>0.0));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("Qd"=>0.0));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("GL"=>0.0));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("BL"=>0.0));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("sh_max"=>0.0));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("sh_min"=>0.0));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("bcs"=>0.0));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("gfM"=>0.0));
        network["bus"][string(i)]=merge(network["bus"][string(i)], Dict("bfM"=>0.0));
        if network["bus"][string(i)]["bus_type"] == 3
        	network=merge(network, Dict("slack"=>i))
        end
    end
    for i=1:length(network["load"])
        I_index=network["load"][string(i)]["load_bus"];
        I=bus_index[string(I_index)];
        ST=network["load"][string(i)]["status"];
        PL=ST*(network["load"][string(i)]["pd"])+network["bus"][string(I)]["Pd"];
        QL=ST*(network["load"][string(i)]["qd"])+network["bus"][string(I)]["Qd"];
        network["bus"][string(I)]=merge(network["bus"][string(I)], Dict("Pd"=>PL, "Qd"=>QL));
        #network["bus"][string(I)]=merge(net_bus[string(I)], Dict("Qd"=>QL));
    end
    for i=1:length(network["shunt"])
        I_index=network["shunt"][string(i)]["shunt_bus"];
        I=bus_index[string(I_index)];
        ST=network["shunt"][string(i)]["status"];
        GL=ST*(network["shunt"][string(i)]["gs"])+network["bus"][string(I)]["GL"];
        BL=ST*(network["shunt"][string(i)]["bs"])+network["bus"][string(I)]["BL"];
        network["bus"][string(I)]=merge(network["bus"][string(I)], Dict("GL"=>GL, "BL"=>BL));
    end
    for i=1:length(network["gen"])
        I_index=network["gen"][string(i)]["gen_bus"];
        I=bus_index[string(I_index)];
        network["gen"][string(i)]=merge(network["gen"][string(i)], Dict("gen_bus_index"=>I));
        network["gen"][string(i)]=merge(network["gen"][string(i)], Dict("gen_bus"=>I_index));
        network["gen"][string(i)]=merge(network["gen"][string(i)], Dict("gov_droop"=>0.0));
        network["gen"][string(i)]=merge(network["gen"][string(i)], Dict("total_apf"=>0.0));
        network["gen_cost_order"] = max(length(network["gen"][string(i)]["cost"]),network["gen_cost_order"]);
    end
    for i=1:length(network["branch"])
	I_index=network["branch"][string(i)]["f_bus"];
	I=bus_index[string(I_index)];
	J_index=network["branch"][string(i)]["t_bus"];
	J=bus_index[string(J_index)];
	network["branch"][string(i)]=merge(network["branch"][string(i)], Dict("f_bus_index"=>I));
	network["branch"][string(i)]=merge(network["branch"][string(i)], Dict("t_bus_index"=>J));
	network["branch"][string(i)]=merge(network["branch"][string(i)], Dict("gfM"=>0.0));
	network["branch"][string(i)]=merge(network["branch"][string(i)], Dict("bfM"=>0.0));
    end
...
function process_raw(network)
    ...
    for g=1:length(network["gen"]) gen=network["gen"][string(g)];
        if (gen["gen_bus_index"]*gen["gen_status"]>0) push!(network["nbal"][string(gen["gen_bus_index"])]["gen"],g); end
    end
...
function add_buses(net_bus)
    bus_index=Dict{String,Any}();
    for i=1:length(net_bus)
        ...
        bus_index=merge(bus_index, Dict(string(net_bus[string(i)]["index"])=>i));
...
function add_Lbranches(net_br, bus_index, BaseMVA, RawString, start, size)
    ...
    for ind=1:size
        ...
        if (St==1) i+=1;
            ...
            net_br[string(i)]=merge(net_br[string(i)], Dict("ratea"=>RATEA));
            net_br[string(i)]=merge(net_br[string(i)], Dict("ratec"=>RATEC));
...
function add_Tbranches(net_bus, net_br, bus_index, BaseMVA, RawString, pre, start, size)
    ...
    for i in 1:4:size
        ...
        if (St==1)  ind+=1;
            ...
            net_br[string(ind+pre)]=merge(net_br[string(ind+pre)], Dict("ratea"=>RATEA1));
            net_br[string(ind+pre)]=merge(net_br[string(ind+pre)], Dict("ratec"=>RATEC1));
